...
const Pagination = ({ usersTotalCount, usersPerPage, currentPage, onPageClick, portionSize = 10 }) => {
  ...
  let pagesAmount = () => {
    let pages = []
    for (let i = 1; i <= pagesTotalCount; i++) { pages.push(i) }
    return pages
  }
  ...
  return <div className={style.paginationContainer}>
    ...
    {
      pagesAmount().filter(p => p >= firstPageOfCurrentPortion && p <= lastPageOfCurrentPortion)
        .map(p =>
          <span
            key={p}
            className={`${style.page} ${currentPage === p ? style.pageActive : ''}`}
